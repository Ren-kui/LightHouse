...
覆盖全结局可达性测试：逐个验证 8 结局的判定条件正确性 + 优先级顺序。
...
class TestEndingReachability(unittest.TestCase):
    ...
    def test_ending_G_all_toys(self):
        self._reset()
    ...
    def test_ending_G_missing_tin_soldier(self):
        self._reset()
        self.im.acquire("music_box")
        self.im.acquire("warm_marble")
        self._set_vars(curiosity=5, sanity=5)
        self.assertNotEqual(self.st.check_ending(), "G")

    def test_ending_G_missing_music_box(self):
        self._reset()
        self.im.acquire("tin_soldier")
        self.im.acquire("warm_marble")
        self._set_vars(curiosity=5, sanity=5)
    ...
    def test_ending_G_missing_warm_marble(self):
        self._reset()
        self.im.acquire("tin_soldier")
        self.im.acquire("music_box")
        self._set_vars(curiosity=5, sanity=5)
    ...
    def test_ending_B_blocked_by_no_pen(self):
        self._reset()
        self._set_vars(curiosity=6, sanity=6, trust=7, loyalty=4)
        self.assertNotEqual(self.st.check_ending(), "B")
    ...
    def test_ending_D_blocked_by_no_textbook(self):
        self._reset()
        self._set_vars(curiosity=3, sanity=6, trust=3, loyalty=2)
        self.assertNotEqual(self.st.check_ending(), "D")
    ...
    def test_priority_D_over_C(self):
        """D（c≤3, s≥6, t≤3, l≤2, 有课本）优先于 C（c≤3, s≥6）"""
        self._reset()
        self.im.acquire("zhang_textbook")
        self._set_vars(curiosity=3, sanity=6, trust=3, loyalty=2)
        self.assertEqual(self.st.check_ending(), "D")
