# 游戏设计文档

> 维护者：GD / ND / UI / FD | 最后更新：2026-05-08
> 本文档是程序开发和剧情编写的唯一权威参考，按栏目分配角色认领。

---

# 第一卷 · 交互逻辑

> 认领角色：**UI**（主责）、FD（配合实现）
>
> [UI] 我负责玩家看到什么、摸到什么、怎么摸。标题界面→逐字文本→箭头选项→滑入面板，四块全部对齐Undertale设计哲学。黑底白字是底色，橙红是唯一强调色。逐字打印让玩家与文本同步呼吸。空格继续是给人类的提示。Tab滑入面板代替弹窗。

---

## 1.1 进入游戏流程

双击启动 -> 引语淡入 -> "灯  塔"标题浮现 -> 1.5秒 -> 菜单：新游戏/继续游戏/退出 -> 第1章逐字打印



## 1.2 核心交互界面

### 标题界面
- 引语(灰色小字) + "灯 塔"(红色标题) + 居中菜单
- 悬停亮橙(#cc6600)，点击触发流程

### 文本阅读界面
- 逐块逐字打印，纯黑底+微软雅黑粗体白色字
- 空格/回车/点击：打字中跳过当前块 / 块结束翻页 / 全部结束显示选项
- 提示："▼ 空格/回车继续"(灰色)

### 选项界面
- ▶ 选项文字，居中排列
- 悬停箭头白+文字橙，点击执行选择

### 状态面板(Tab呼出)
- 主窗口变暗 → 右侧滑入32%宽度，笔记风格隐晦状态+物品悬停

### 存档/读档
- 纯黑浮窗，3手动+1自动，▶ 箭头选项

---

## 1.3 UI风格规范

| 元素 | 规格 |
|------|------|
| 背景 | #000000 纯黑 |
| 主文字 | #e8e8e8 微软雅黑粗体13px |
| 强调文字 | #cc0000 血红 |
| 选项默认 | #666666 灰箭头+灰字 |
| 选项悬停 | #e8e8e8 白箭头+#cc6600 暗橙字 |
| 提示 | #555555 灰色10px |
| 窗口 | 860x640，最小640x480 |

---

## 1.4 UI动画

### 逐字打印
- 30ms/字，按\n\n分块，块间按键翻页

### 红色抖动文字
- #cc0000 + 水平+-3px偏移 12.5次/秒，持续5秒，每章<=3次

### 多次点击按钮
- 连点3-5次，文字逐次变化，颜色渐红

### 小游戏界面（B1）
- 居中 700×550 固定框架，外框 #555555 3px，全屏黑色遮罩
- 小游戏 Canvas 自动填满内层区域

---

# 第二卷 · 功能与玩法设计

> 认领角色：**GD**（主责）
>
> [GD] 玩法核心是选择改变命运。3显性变量影响选项出现消失，2隐藏变量在关键节点引爆。小游戏升级为叙事工具(黑暗收缩失败即死)。5结局加特殊死亡，变量组合唯一判定。

---

## 2.1 核心循环

阅读文本 -> 2-4选项 -> 玩家选择 -> 变量更新 -> 跳转节点 -> 循环

**结构特点**：主线是一条固定的线性叙事。选项不产生剧情分叉——它们改变的是变量值。所有路径最终汇聚回同一个主线节点。**选择后的不同体验由 `text_bridges` 承担**：类型一桥承载分叉叙事（冲突内容移入桥），类型二桥只写体验过渡。详见 `docs/bridges.md`。

---

## 2.2 选择类型

| 类型 | 说明 |
|------|------|
| 普通 | 无条件箭头选项 |
| 条件 | 变量满足阈值才显示 |
| 抗拒 | 连点3-5次，multi_click |
| 逃避 | 小游戏前"自己做/队友代劳" |
| 无选择 | 结局/死亡/过渡 |

---

## 2.3 变量系统

| 变量 | 类型 | 范围 | 初始 |
|------|------|------|------|
| curiosity 好奇心 | 显性 | 0~10 | 0 |
| sanity 理智 | 显性 | 0~10 | 10 |
| trust 信任 | 显性 | 0~10 | 4 |
| survival_will 生存意愿 | 隐藏 | 0~10 | 6 |
| loyalty 忠诚 | 隐藏 | 0~5 | 3 |

判定：直接分流 / 阈值判定 / 组合判定

### 第 4-5 章关键 effects 表（GD + ND 对齐）

| 节点 | 条件/选择 | effects | 备注 |
|------|----------|---------|------|
| ch04 张海生坦白 | trust≥6 → 他告诉我了 | trust+1, curiosity+1 | 信息路径 |
| | trust<6 → 他还在瞒我 | trust-1, survival_will+1 | 隐瞒路径 |
| ch04 梦境 | "跟着父亲走" | curiosity+2, sanity-2 | 塔的诱惑 |
| | "转身醒来" | survival_will+2, sanity-1 | 对抗诱惑 |
| ch04 MG4-A 海鸟 | 成功 | survival_will+1, trust+1 | 扛住了 |
| | 失败 | sanity-2 | 被击中 |
| ch04 隐藏道具 | survival_will≥8 | flag: found_dad_logbook | 黑暗收缩终点 |
| | curiosity≥7 | flag: found_wangchao_drawing | 水彩画浮现 |
| ch04 MG4-B | survival_will≥8 | 难度降一档 | 意志坚定 |
| | loyalty≤2 | 难度升一档 | 孤立无援 |
| ch05 日记撕页 | loyalty≥4 → 展示 | flag: found_diary_page, trust+1 | 张海生坦白 |
| | loyalty<4 → 隐瞒 | trust-1 | 保持距离 |
| ch05 合作/对抗 | trust≥6+loyalty≥4 | trust+1, loyalty+1（合作） | 约好一起对抗 |
| | trust≤4+loyalty≤3 | trust-1, survival_will+1（对抗） | 独自求生 |
| ch05 离开选项 | survival_will≥8+sanity≤4+trust≤4 | → ch05_early_escape（结局E入口） | 提前逃离 |
| ch05 MG5 | 合作路线 | 张海生辅助 | V2 不对称收缩 |

---

## 2.4 小游戏体系

| 编号 | 名称 | 天 | 成功 | 失败 | 逃避 | 规格 |
|------|------|----|------|------|------|------|
| MG1 | 配电连线 | D2 | +信任 | -信任 | -信任 | 6对端子+6中继点+亮灭灯循环(亮1.5s/灭2.5s)，左→中继→右三段配对，灭灯可操作，36s时限 |
| MG2 | 太阳能反应 | D2 | +信任 | -信任 | -信任 | 黄点缩小至50%+蓝干扰点(0-2个，点中-1)+移动+15%双黄点，目标6/12 |
| MG3 | 平台平衡 | D4 | +信任 | -理智 | -忠诚 | 持续点击保持光标稳定在中心区域——20s 时限 |
| MG4A | 海鸟躲避 | D9 | +survival_will/+trust+头骨flag | -理智 | 不可逃 | 25s计时（被击中≤4次胜利），灰鸟1x/红鸟2.5x(1/15概率)，鼠标归正中心，`found_bird_skull` flag |
| MG4B | 黑暗收缩复合 | D9 | 上下屏皆胜 | 死亡 | 不可逃 | 上屏6对配电+下屏暗红收缩+能量条(30格/-1每2s/点击+1/连击10过热4s冷却/归零收缩×3/收缩≥1死亡) |
| MG5 | 黑暗收缩复合 | D12 | 同上 | 死亡 | 不可逃 | 同MG4B，时限45s |

---

## 2.5 结局

### 判定优先级

三玩具集齐 > MG4/MG5死亡 > 变量组合+物品判定

### 结局触发条件（GD + FD 对齐 · 2026-05-15 · D047）

| 优先级 | ID | 结局 | 变量条件 | 物品条件 | 路径特征 |
|--------|----|------|---------|---------|---------|
| 1 | **G** | 荒诞 · 被带走 | — | tin_soldier + music_box + warm_marble 全齐 | 三件不合时宜的小玩具集齐→直接触发 |
| 2 | **death** | 灯塔未解死亡报告 | — | — | MG4-B 或 MG5 失败 |
| 3 | **F** | 被背叛 · 死在黑暗里 | curiosity≥7 + sanity≤3 + trust≤3 + loyalty≤2 | 无 | 高好奇 + 全选降信任/降忠诚 |
| 4 | **A** | 疯狂 · 独自对话 | curiosity≥7 + sanity≤3 + (trust≥5 或 loyalty≥3) | 无（evil_wood_carving 可助推 sanity） | 高好奇但保留人际关系 |
| 5 | **E** | 提前逃离 · 永不知答案 | survival_will≥8 + sanity≤4 + trust≤4（ch05 特殊节点） | 无 | 高生存 + 低信任 |
| 6 | **B** | 一起逃离 · 他断开关 | curiosity≥6 + sanity≥6 + trust≥7 + loyalty≥4 | pen（纯黑钢笔） | 高好奇但避开伤害理智 |
| 7 | **D** | 被杀 · 离开前一夜 | curiosity≤3 + sanity≥6 + trust≤3 + loyalty≤2 | zhang_textbook（张望潮物理课本） | 保守孤立 |
| 8 | **C** | 平安离开 · 什么也没意识到 | **无（万能结局——所有条件不满足时默认进入）** | 无 | 兜底结局 |

### 物品体系

详见 §2.7 物品系统。物品作为结局的必要条件（AND 关系），与变量组合判定共同决定结局归属。部分物品携带变量效果，在面板中显示但不修改 StateManager 实值。

### 结局表现形式

**递进链**：每个结局的正文由 3-5 个纯文字节点 + 单选"继续..."构成（`is_ending_chain: true`）。引擎进入结局链后切换到只读模式——禁面板/Tab/存档/GM。末节点显示结局名称画面（P3-6）。

### 结局名称展示

结局链最后一帧显示：结局名称（大号 #cc0000）+ 结局描述（一行小字 #888888）+ D14 日记（如适用）。

### §2.5.1 结局文案四原则（ND-EP1~EP4 · D049）

> 认领角色：**ND**（主责）、**GD**（结局条件设计协同）
>
> 2026-05-17 ND 全章审校后提炼。详见 `docs/ending_comparison.md`。

| 编号 | 原则 | 说明 |
|:----:|------|------|
| **ND-EP1** | 信息密度与主题一致 | 每个结局揭示的世界观信息量须与其主题相称。逃避型结局不应比探索型结局揭示更多；最优结局应获最大情感回报。 |
| **ND-EP2** | 情感差异可辨识 | 相同变量区间的相邻结局必须在阅读感受上有可辨识差异。不能两个结局都读起来像"被黑暗吞没"。 |
| **ND-EP3** | 文本量与路径成本匹配 | 结局文本的信息量和情感回报应与触发难度成正比。高门槛结局应有更多回报。 |
| **ND-EP4** | 张海生终局状态明确 | 每个结局须明确张海生的终局状态（存/亡/去向）和与玩家的关系收束（断裂/完成/空白/终结）。不允许模糊叙事。 |

**当前已知偏离**：
- E 结局（EP1 偏离）：独占父亲航海真相，信息量全结局第一，但主题是"永不知答案"
- B 结局（EP3 偏离）：最高变量门槛，但文本量和信息密度偏低
- A/F 结局（EP2 偏离）：相邻变量区间，阅读感受趋同

---

## 2.6 存档

4手动位 + 1自动位（独立区域，上下排列）。自动位每次节点切换自动覆盖。

---

## 2.7 物品系统

> 认领角色：**GD**（物品定义+结局关联）、**ND**（物品描述文案）、**FD**（ItemManager 实现）

### 架构

```
src/item_manager.py    ItemManager 类（acquire/remove/has/carry_effects/consume/destroy）
data/items.json        物品定义（id/name/desc/carry_effects/obtain/destroy_quest）
```

物品独立于 StateManager（5 变量）和 StoryEngine（flag 系统）管理。

### 物品清单（7 件 · D048）

| ID | 名称 | 获取 | 携带效果 | 关联结局 |
|----|------|------|---------|---------|
| `key` | 父亲的钥匙 | 开局自动 | survival_will +2 | 起点道具 |
| `tin_soldier` | 不锈的锡兵 | ch02 探索 6 楼墙缝 | — | G 玩具① |
| `music_box` | 自鸣八音盒 | ch03 地下室·选择拿走八音盒（与拆墙/离开三选一） | survival_will -1 | G 玩具② |
| `warm_marble` | 暖弹珠 | ch04 D8 梦境醒·选项1（与 evil_wood_carving 互斥） | — | G 玩具③ |
| `evil_wood_carving` | 邪恶木雕 | ch04 D8 梦境醒·选项2（与 warm_marble 互斥） | sanity -2 | A/F 助推 |
| `pen` | 纯黑钢笔 | 闲逛任务全清后插入叙事获得 | — | B 必要条件 |
| `zhang_textbook` | 张望潮的物理课本 | ch03 夹层·隐藏选项（loyalty≤2） | — | D 必要条件 |
| `wangchao_drawing` | 张望潮的水彩画 | ch04 D8 重返第一天抽屉（curiosity≥7） | curiosity +1 | 叙事回溯 |

### 相互制约关系

- `warm_marble` 与 `evil_wood_carving` 互斥：ch04 D8 梦境醒后三选一（含"直接起床不拿任何东西"），选了一个另一个永久错过。
- `warm_marble` 为 G 结局三玩具之一；`evil_wood_carving` 推进 A/F 结局方向。
- `tin_soldier` + `music_box` + `warm_marble` 三件全齐 → 结局 G（替代旧 flags 三件套）。

### 面板渲染

面板"随身物品"区域从 ItemManager 动态渲染：▸ 物品名 + 悬停显示描述 + 携带效果（如 `生存意志+2`）。空时显示"（空无一物）"。



---

# 第三卷 · 剧情设计

> 认领角色：**ND**（主责）、GD（配合变量）
>
> [ND] 白描>渲染，暗示>直说。江临求生vs寻父的矛盾驱动选择。张海生丧子创伤+日记规则构成关键抉择动机。父子投射用动作暗示不点破。日记残页4段逐步揭示。

---

## 3.1 核心故事引擎

两个普通人在与世隔绝的灯塔上，逐渐分不清超自然现象与自我崩溃的边界。
叙事：白描为主，细节不对劲，让玩家自己脑补恐惧。

---

## 3.2 角色

### 江临(27岁,玩家)
- 17岁父海难失踪，10年梦见父亲从海底走来
- 矛盾：想活下去 vs 想见到父亲
- 幼年湖边站在水里产生"没人能碰到你"的冰冷平静
- 灯塔选择了他(张海生十年无D级接触，江临第一天即A级)

### 张海生(48岁,常驻)
- 前海军救捞员，妻亡，子张望潮2020年在灯塔失踪
- 发现1930年代日记：灯塔需"替身"。撕去关键页自留。
- 困境：灯塔对江临开放了。是否献祭他换取沉默？

---

## 3.3 双人关系

| 层 | 内容 | 表现 |
|----|------|------|
| 表面 | 守塔人+临时工 | D1-14日常对白 |
| 中层 | 信任建立/瓦解 | D3-10行为暗示 |
| 深层 | 父子投射 | D5-14动作闪回暗示，不点破 |

---

## 3.4 章节脉络

| 章 | 天 | 核心 | 玩法 |
|----|----|------|------|
| 1 | D1 | 抵达认识 | 首个选择 |
| 2 | D2-3 | 初现低语 | MG1+MG2 |
| 3 | D4-6 | 发现异层 | MG3 |
| 4 | D7-9 | 抉择危机 | MG4 |
| 5 | D10-12 | 对抗真相 | MG5 |
| 6 | D13-14 | 5结局 | 纯文本 |

---

## 3.5 14天流程

| 天 | 章 | 类型 | 要点 |
|----|----|------|------|
| 1 | 1 | 纯剧情 | 抵达，张海生接待，首个选择支 |
| 2 | 2 | 小游戏 | 配电连线+太阳能测试，可选逃避 |
| 3 | 2 | 纯剧情 | 第一次低语，水下轮廓，红色抖动 |
| 4 | 3 | 小游戏 | 平台维修高空作业 |
| 5 | 3 | 纯剧情 | 发现额外层/地下室入口 |
| 6 | 3 | 纯剧情 | 异常空间探索，队友遗留物 |
| 7 | 4 | 纯剧情 | 队友坦白/对抗，回忆闪回 |
| 8 | 4 | 纯剧情 | 异常升级，主角质疑理智 |
| 9 | 4 | 小游戏 | 停电，黑暗收缩死亡场景 |
| 10 | 5 | 纯剧情 | 深度探索，队友命运分岔 |
| 11 | 5 | 纯剧情 | 过去被异常投射，必须面对记忆 |
| 12 | 5 | 小游戏 | 最终对抗黑暗收缩 |
| 13 | 6 | 纯剧情 | 倒计时，结局判定 |
| 14 | 6 | 结局 | A/B/C/D/E 或特殊死亡 |

---

## 3.6 关键双视角表

| 节点 | 天 | 江临 | 张海生 |
|------|----|------|--------|
| 问海浪 | D2 | 好奇试探 | 警觉不说 |
| 发现异层 | D5 | 惊恐被吸引 | 握拳，救不救？ |
| 找到日记 | D7 | 他为什么撕？ | 撕掉是让他看懂 |
| 停电 | D9 | 必须活下去 | 灯塔要他 |
| 高忠诚 | D12 | 比朋友多 | 我选对了 |
| 低忠诚 | D12 | 他去断开关？ | 安静锁门 |

---

## 3.7 灯塔异常映射

| 异常 | 江临 | 张海生 |
|------|------|--------|
| 低语 | 父亲方言 | 永是海浪 |
| 轮廓 | 巨影+父背影 | 从未见 |
| 异空间 | 墙上字似父手迹 | 牛奶杯原样 |
| 被选 | 是 | 否 |

---

## 3.8 日记残页

位置：底层储藏室铁盒(curiosity>=3发现)，4段揭示，张海生撕去关键页藏床头

---

## 3.9 结局状态（2026-05-17 ND 更新 · D049）

| 结局 | 江临 | 张海生 | 关系 | 终局信息 |
|------|------|--------|------|---------|
| A | 留下与塔对话 | 搭肩后离开，去向不明 | 断裂 | 塔以螺丝/温差/墙字为语言；"它问你，你回答了" |
| B | 清醒离开，承诺回来 | 站在天台目送 | 完成 | 未揭示额外世界观 |
| C | 平安离开，什么都没做 | 正常握手告别 | 空白 | 零世界观揭示 |
| D | 离开后又回头，发现他已死 | 死亡——掌心是空的 | 终结 | 他留了话但"你只是没有去听"；偷课本触发 |
| E | 提前逃离，放弃知道 | 留守，在日志中补写留话 | 永别 | 父亲二十多年前知道塔、来过、未归——信息量最大 |
| F | 被黑暗接走 | 未直接出现（声音被塔伪造） | 断裂 | "它不能抓只能接""它接了" |
| G | 走下海底，被塔吸收 | 旁观，不阻止 | 终结（失去第三人） | 三个水中人影=父亲/守塔人/张望潮；"它有你" |
| death | 失踪 | 存活但"拒绝陈述" | 无声 | 调查报告格式；墙上新刻"我不曾离开" |

> **ND-EP4 注**：A/F/death 中张海生出场偏弱；A 中"他转身走下楼梯"去向未交代；D 中"第三个人"概念需在别处呼应。

---

# 第四卷 · 程序设计

> 认领角色：**FD**（主责）
>
> [FD] 9模块引擎：main.py协调全部；main_window.py Undertale全GUI；status_panel.py右侧滑入；story_engine.py驱动分支；state_manager.py管5变量；save_manager.py管存档；effects.py特效；darkness_overlay.py收缩V1；minigame_base.py基类。

---

## 4.1 模块架构

```
main.py               Game类入口 + FSM
  main_window.py      Undertale主窗口 (含标题/文本/选项/面板/存档/GM/小游戏覆盖层/结局画面)
  story_engine.py     JSON→分支+条件判定+物品判定
  state_manager.py    5变量+描述+条件
  save_manager.py     4手动1自动存档
  sound_manager.py    纯代码合成音效(SD)
  item_manager.py     物品管理：7件物品CRUD+carry_effects+结局穿透
  minigame_base.py    MG1/MG2/MG3/MG4A/MG4B5 小游戏
  darkness_overlay.py 黑暗收缩叠加层(V1/V2)
```

### 数据文件

```
data/
  chapter_01~06.json   剧情数据（ND 维护）
  diary.json           日记文本（ND 维护）
  diary_fragments.json 日记残页原文（ND 维护）
  items.json           物品定义（GD 定义物品+结局关联，ND 写描述文案）
  markup_registry.json 标记注册表（GD 维护）
```

---

## 4.2 JSON格式

node_id/chapter/day/mood(可选:dread/tension/loss/quiet,缺省=关闭)/text(按\n\n分块)/choices(effects+conditions+multi_click+important(关键选项金色显示,#ccaa00))/auto_next/is_chapter_end/is_ending_chain(结局链标记,单选自动推进+只读模式)/text_bridges(按前置节点匹配承接桥段——详见 `docs/bridges.md` 写作原则与配置表)

### 日记系统 (diary.json)
每天4句话（理智/好奇/信任/生存），各分高/低两版，按变量阈值自动选分支。`‡关键词‡` 渲染为暗绿色(#008800)+下划线。阈值：sanity≤4(低)/curiosity≤3(低)/trust≤4(低)/survival_will≤5(低)。ND 维护文本，FD 引擎接入。

---

## 4.3 已实现

全部12模块 + GM调试栏 + 6章节JSON + MG1~MG5全部优化(B1~B5) + 24选项重要标记金色显示 + 97测试全绿

---

## 4.4 待实现

exe打包 | 全结局可达性测试 | 错别字审查 | ~~darkness_overlay~~ ✅ | ~~多次点击渐变~~ ✅ | ~~[pause]解析~~ ✅ | ~~multi_click~~ ✅ | ~~sound_state存档~~ ✅ | ~~MG1~MG5全部优化~~ ✅

---

## 4.5 音效模块 (sound_manager.py)

> 认领角色：**SD**（主责）、FD（挂载接口）
>
> [SD] 极简主义：每章 ≤3 种声音事件。低频 + 留白 > 频繁打断。音效服务于叙事焦虑，不抢文本注意力。全部纯代码合成（wave + struct），零外部依赖/零音频素材文件。

### API 接口

| 方法 | 说明 |
|------|------|
| `play(name)` | 播放命名音效（如 "hum_low"、"heartbeat"） |
| `stop(name)` | 停止指定音效 |
| `stop_all()` | 静默全部音效 |
| `set_volume(0.0~1.0)` | 全局音量控制，带边界保护 |
| `mute()` / `unmute()` | 静音开关 |

### 音效注册表

| 名称 | 频率/波形 | 时长 | 循环 | 触发场景 |
|------|-----------|------|------|----------|
| `hum_low` | 40Hz 正弦波 | 循环 | 是 | day≥3 夜间节点自动触发，底噪氛围 |
| `heartbeat` | 双脉冲 2Hz | 循环 | 是 | [shake] 红色抖动同步，黑暗收缩期间 |
| `metal_creak` | 高频方波 | 0.3s | 否 | 灯塔金属收缩，特定剧情节点标记 |
| `water_drone` | 滤波白噪声 | 循环 | 是 | 凌晨/夜晚水下场景，低音量环境底噪 |
| `silence_fade` | 当前音效淡出 | 0.5s | 否 | 节点切换、章节过渡、面板打开时触发 |

### 静默规则

| # | 条件 | 行为 |
|---|------|------|
| 1 | 章节过渡 / 节点切换 | `silence_fade` 淡出旧音效 0.5s |
| 2 | 面板打开（Tab） | 当前音效淡出到 0，面板关闭后恢复 |
| 3 | 选项出现 | 音量降至 20%，选择后恢复 |
| 4 | 结局判定 / 死亡 | 全部音效立即停止，静默 1.5s → 文本浮现 |

### 自动化触发

环境音效（`hum_low`、`water_drone`）由引擎根据当前 chapter/day 自动判定触发，ND 无需在 JSON 中手动标记。事件音效（`heartbeat`、`metal_creak`）由 ND 在 JSON text 中使用 `[sound name]` 标记，生命周期与所在 `\n\n` 文本分块绑定——块内触发、块外停止。

### 存档格式

```json
"sound_state": {
  "active": "hum_low",
  "volume": 0.8,
  "muted": false
}
```

---

# 第五卷 · 角色任务清单

## UI

| # | 任务 | 优先 | 状态 |
|----|------|------|------|
| UI-01~05 | 窗口+标题+选项+打印+面板 | P0 | 完成 |
| UI-06 | 存档读档浮窗 | P1 | ✅ 已纳入主窗口 |
| UI-07 | 红色抖动调优 | P1 | ✅ |
| UI-08 | 多击按钮渐变 | P2 | ✅ |

## GD

| # | 任务 | 优先 | 状态 |
|----|------|------|------|
| GD-01~05 | 变量+结局+节奏+小游戏+关系 | P0 | 完成 |
| GD-06 | 隐藏结局E条件 | P2 | 待 |
| GD-07 | 按钮新玩法方案 | P2 | 待 |

## ND

| # | 任务 | 优先 | 状态 |
|----|------|------|------|
| ND-01~02 | 角色全传+第1章JSON | P0 | 完成 |
| ND-03 | 日记4段原文 | P0 | 完成 |
| ND-04~08 | 第2~6章JSON | P1 | 进行中 |
| ND-09~11 | 闪回+逃避对白+隐藏E | P2 | 待 |

## FD

| # | 任务 | 优先 | 状态 |
|----|------|------|------|
| FD-01~04 | 架构+UI+打印+流程 | P0 | 完成 |
| FD-05 | 5小游戏实现+全面优化(B1~B5) | P1 | ✅ |
| FD-07 | 多击按钮视觉 | P1 | ✅ |
| FD-09 | 存档浮窗 | P1 | 完成 |
| FD-06/08 | 黑暗收缩集成/[shake]解析 | P2 | ✅ |
| FD-10 | exe打包 | P3 | 待 |
| FD-11 | 第2-3章JSON | P1 | 完成 |
| FD-12 | 挂载 SoundManager 到 Game.__init__（M4 空壳） | P1 | 完成 |

## QA

| # | 任务 | 优先 | 状态 |
|----|------|------|------|
| QA-01~04 | 第1章分支+变量+存档+提示 | P1 | 待 |
| QA-05 | 第2-6章(每章后) | P2 | 待 |
| QA-06~08 | 全结局+错字+打包测试 | P3 | 待 |

## SD

| # | 任务 | 优先 | 状态 |
|----|------|------|------|
| SD-01 | sound_manager.py 空壳 API + sound_spec.md | P1 | 完成 |
| SD-02 | 5 核心音效预生成实现 | P2 | 待 |
| SD-03 | [sound] 标记引擎解析 + ND 触发点标注 | P2 | 待 |
| SD-04 | 音效时序调优 + exe 打包验证 | P3 | 待 |

---

> **P0**=已完成 **P1**=下一批必须完成 **P2**=后续批次 **P3**=最终打磨

