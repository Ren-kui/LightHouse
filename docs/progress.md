...
| 多击按钮颜色渐变 — 已由 M5 完成 | FD+UI | ✅ |
| ~~[sound] 标记引擎解析~~ | ~~FD+SD~~ | ~~暂停~~（音效全局暂停） |
| 全结局可达性测试 | QA | 待 |
| 错别字审查 | QA/ND | 待 |
| ~~音效时序调优~~ | ~~SD~~ | ~~暂停~~（音效全局暂停） |
| exe 打包 + 运行测试 | FD+QA | 待 |
...
> **当前状态**：M5 ✅ 100%。M4 ✅。87 测试全绿。音效全局暂停。M5 全部完成：8 结局判定+flag 系统、全章 JSON+桥段（类型 I/II）、MG1~MG5（darkness V1 交互 + V2 不对称收缩）、多击按钮颜色渐变。→ 进入 M6。
